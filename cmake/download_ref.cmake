...
if(IS_DIRECTORY ${ref_dir})
  if(hash AND EXISTS ${ref_dir}/sha256sum.txt)
    ...
    if(${ext_hash} STREQUAL ${hash})
      message(STATUS "${name}: extracted hash == JSON hash, no need to download.")
      return()
    else()
      message(STATUS "${name}: extracted hash ${ext_hash} != JSON hash ${hash}")
    endif()
  else()
    message(STATUS "${name}: JSON hash not given and ${ref_dir} exists, no need to download.")
    return()
  endif()
endif()
...
if(EXISTS ${archive} AND DEFINED hash)
  ...
  if(${archive_hash} STREQUAL ${hash})
    message(STATUS "${name}: archive hash == JSON hash, no need to download.")
  else()
